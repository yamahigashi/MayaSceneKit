...
use super::bundle::MaObservationData;
use crate::scene::{ScenePathEntry, ScenePathMeta, ScriptNodeEntry};

impl MaObservationData {
    pub(super) fn top_level(&self) -> &mel::MelAuditTopLevelFacts {
        &self.selective_sections().audit_top_level
    }

    pub(super) fn dump_sections(&self) -> &RawMaDumpSections {
        &self.selective_sections().dump_sections
    }

    pub(super) fn script_entries(&self) -> &[RawMaScriptEntry] {
        self.dump_sections().script_entries.as_slice()
    }

    pub(super) fn scene_paths(&self) -> &[ScenePathEntry] {
        self.scene_paths
            .get_or_init(|| {
    ...
    pub(super) fn selective_sections(
        &self,
    ...
    pub(super) fn bytes(&self) -> Result<&[u8], crate::scene::SceneToolError> {
...
pub(super) fn collect_ma_script_entries(entries: &[RawMaScriptEntry]) -> Vec<ScriptNodeEntry> {
    entries
        .iter()
...
#[cfg(test)]
pub(super) fn collect_ma_scene_paths(bytes: &[u8]) -> Vec<ScenePathEntry> {
