...
use super::{
    super::context::SchemaInputs, ExecutionSurface, ObservedExecutionCatalog,
    ObservedExecutionSurface, catalog, ma, mb,
};
use crate::{
    mb::MbParseBudget,
    scene::{
        DependencyFact, ExecutionUnitSummary, PathKind, SceneDigestSet, SceneDumpRequireEntry,
        SceneDumpRequireKind, SceneFormat, ScenePathEntry, SceneToolError, ScriptNodeEntry,
        ValidationState, mb_extract, mb_read_session::MbReadSession, ops,
    },
};
...
impl Loader {
    ...
    pub fn observe_path_without_retained_ma_bytes(
        &self,
        path: impl AsRef<Path>,
    ) -> Result<ObservationBundle, SceneToolError> {
...
pub struct ObservationBundle {
    ...
    pub(super) data: ObservationData,
}

pub(super) struct MaObservationData {
    pub(super) source_path: PathBuf,
    pub(super) bytes: OnceLock<Vec<u8>>,
    pub(super) selective_sections: OnceLock<RawMaSelectiveSections>,
    pub(super) scene_paths: OnceLock<Vec<ScenePathEntry>>,
}

pub(super) enum ObservationData {
...
impl ObservationBundle {
    ...
    pub fn script_node_entries(&self) -> Result<Vec<ScriptNodeEntry>, SceneToolError> {
        match &self.data {
            ObservationData::Ma { data } => {
                Ok(ma::collect_ma_script_entries(data.script_entries()))
            }
    ...
    pub fn scene_dump_report(&self) -> Result<crate::scene::SceneDumpReport, SceneToolError> {
        let require_entries = self.require_entries()?;
        Ok(crate::scene::SceneDumpReport {
            scene_path: self.scene_path().to_path_buf(),
            ...
            requires: require_entries
                ...
                .collect(),
            require_entries,
            script_entries: self.script_node_entries()?,
        })
    }
    ...
    pub fn requires(&self) -> Result<Vec<String>, SceneToolError> {
        match &self.data {
            ...
            ObservationData::Mb { session } => {
                Ok(mb_extract::extract_requires_from_mb(&session.mb))
            }
        }
    }
    ...
    pub fn require_entries(&self) -> Result<Vec<SceneDumpRequireEntry>, SceneToolError> {
        match &self.data {
            ObservationData::Ma { data } => Ok(data
                ...
                .collect()),
            ObservationData::Mb { session } => {
                Ok(mb_extract::extract_require_entries_from_mb(&session.mb))
            }
        }
    }
    ...
    #[cfg(test)]
    pub(super) fn cached_execution_core_ptr(
        &self,
    ...
    #[cfg(test)]
    pub(super) fn cached_scene_digests_ptr(&self) -> Option<*const SceneDigestSet> {
        self.scene_digests.get().map(std::ptr::from_ref)
    }

    #[cfg(test)]
    pub(super) fn cached_ma_bytes_ptr(&self) -> Option<*const Vec<u8>> {
        match &self.data {
...
fn collect_mb_script_entries(nodes: &[crate::scene::ir::RecoveredNode]) -> Vec<ScriptNodeEntry> {
    ...
    for node in nodes {
        ...
        for attr in &node.attrs {
            ...
            let crate::scene::ir::SetAttrValue::String(body) = &op.value else {
                continue;
            };
        ...
        entries.extend(bodies.into_iter().map(|body| ScriptNodeEntry {
            name: node.name.clone(),
            body,
        }));
    }
