#[cfg(test)]
use super::parse_ascii_scene;
#[cfg(test)]
use super::parse_support::token_text_required;
use super::parse_support::{normalized_arg_text, parse_numeric_token};
#[cfg(test)]
use crate::ma::{
    commands::{Token, bare_token, token_text},
    values::{parse_bool_token, parse_u32_token},
};
use crate::{
...
fn parse_optional_bool_flag(
    ...
    idx: usize,
    flag: &str,
) -> Result<(bool, usize), SceneToolError> {
    match tokens.get(idx + 1).and_then(bare_token) {
        Some(next_flag) if next_flag.starts_with('-') => Ok((true, 1)),
        None => Ok((true, 1)),
        _ => Ok((parse_bool_token(tokens.get(idx + 1), flag)?, 2)),
    }
}
...
#[cfg(test)]
pub(super) fn parse_add_attr_command(tokens: &[Token]) -> Result<ParsedAddAttr, SceneToolError> {
    let mut short_name = None;
    let mut long_name = None;
    let mut parent = None;
    let mut number_of_children = None;
    ...
    while idx < tokens.len() {
        ...
        match flag {
            "-sn" => {
                short_name = tokens.get(idx + 1).and_then(token_text).map(str::to_string);
                idx += 2;
            }
            "-ln" => {
                long_name = tokens.get(idx + 1).and_then(token_text).map(str::to_string);
                idx += 2;
            }
            "-p" => {
            ...
            "-nc" => {
                number_of_children = Some(parse_u32_token(tokens.get(idx + 1), "-nc")?);
                idx += 2;
            }
            "-nn" => {
                nice_name = tokens.get(idx + 1).and_then(token_text).map(str::to_string);
                idx += 2;
            }
            "-dv" => {
                default_value = Some(parse_numeric_token(token_text_required(
                    tokens.get(idx + 1),
                    "addAttr -dv",
                )?)?);
                idx += 2;
            ...
            "-min" => {
                min_value = Some(parse_numeric_token(token_text_required(
                    tokens.get(idx + 1),
                    "addAttr -min",
                )?)?);
                idx += 2;
            ...
            "-max" => {
                max_value = Some(parse_numeric_token(token_text_required(
                    tokens.get(idx + 1),
                    "addAttr -max",
                )?)?);
                idx += 2;
            ...
            "-smn" => {
                soft_min_value = Some(parse_numeric_token(token_text_required(
                    tokens.get(idx + 1),
                    "addAttr -smn",
                )?)?);
                idx += 2;
            ...
            "-smx" => {
                soft_max_value = Some(parse_numeric_token(token_text_required(
                    tokens.get(idx + 1),
                    "addAttr -smx",
                )?)?);
                idx += 2;
            ...
            "-en" => {
                enum_names = tokens.get(idx + 1).and_then(token_text).map(str::to_string);
                idx += 2;
            }
            "-uac" => {
                let (value, consumed) = parse_optional_bool_flag(tokens, idx, "-uac")?;
                used_as_color = value;
                idx += consumed;
            }
            "-ci" => {
                let (value, consumed) = parse_optional_bool_flag(tokens, idx, "-ci")?;
                cached_internally = value;
                idx += consumed;
            }
            "-h" => {
                let (value, consumed) = parse_optional_bool_flag(tokens, idx, "-h")?;
                hidden = value;
                idx += consumed;
            }
            "-k" => {
                let (value, consumed) = parse_optional_bool_flag(tokens, idx, "-k")?;
                keyable = value;
                idx += consumed;
            }
            "-m" => {
                let (value, consumed) = parse_optional_bool_flag(tokens, idx, "-m")?;
                multi = value;
                idx += consumed;
            }
            "-im" => {
                let (value, consumed) = parse_optional_bool_flag(tokens, idx, "-im")?;
                index_matters = value;
                idx += consumed;
            }
            "-at" => {
                let attr_type = tokens.get(idx + 1).and_then(token_text).ok_or_else(|| {
                    SceneToolError::Message("addAttr -at missing type token".to_string())
                })?;
                value_spec = Some(AddAttrValueSpec::AttrType(attr_type.to_string()));
                idx += 2;
            ...
            "-dt" => {
                let data_type = tokens.get(idx + 1).and_then(token_text).ok_or_else(|| {
                    SceneToolError::Message("addAttr -dt missing type token".to_string())
                })?;
                value_spec = Some(AddAttrValueSpec::DataType(data_type.to_string()));
                idx += 2;
