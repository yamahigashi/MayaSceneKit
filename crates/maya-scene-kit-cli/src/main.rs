pub mod scene {
    ...
    pub fn audit_script_nodes_with_options(
        ...
        maya_scene_kit_audit::audit::audit_script_nodes_with_options(
            path,
            plan,
            load_options,
            options,
...
mod tests {
    ...
    fn script_dump_ma_text_contains_expected_sections() {
        ...
        let text = scene::render_script_dump(&source).expect("script dump");
    ...
    fn requires_dump_mb_text_contains_expected_sections() {
        ...
        let text = scene::render_requires_dump(&source).expect("requires dump");
    ...
    fn scene_dump_writer_emits_combined_dump_file() {
        ...
        cli::fs::write_scene_dump(&source, &output, &scene::LoadOptions::default())
    ...
    fn scene_dump_report_formats_existing_sections() {
        ...
        let text = scene::render_scene_dump_from_report(&report);
    ...
    fn requires_dump_rejects_unknown_scene_format_with_typed_error() {
        ...
        let err = scene::render_requires_dump(&source).expect_err("expected unsupported format");
        assert!(matches!(
            err,
            scene::SceneToolError::UnsupportedSceneFormat {
                detected: scene::SceneFormat::Unknown,
                ..
            }
        ));
    }
