use crate::{
    ...
    PathPromptOptions, PlatformKeyboardLayout, PlatformKeyboardMapper, PlatformTextSystem,
    PromptButton,
...
pub(crate) struct TestPrompts {
    ...
    paths: VecDeque<(
        PathPromptOptions,
        oneshot::Sender<Result<Option<Vec<std::path::PathBuf>>>>,
    )>,
}
...
impl TestPlatform {
    ...
    pub(crate) fn simulate_path_prompt_selection(
        &self,
...
impl Platform for TestPlatform {
    ...
    fn prompt_for_paths(
        ...
        let (tx, rx) = oneshot::channel();
        self.background_executor()
            .set_waiting_hint(Some(format!("PROMPT FOR PATHS: {:?}", options.prompt)));
