...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        let handled = match msg {
            ...
            WM_SYSCOMMAND => self.handle_system_command(handle, wparam, lparam),
    ...
    fn handle_syskeydown_msg(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        if is_alt_space_syskeydown(wparam, current_modifiers())
            && self.show_system_menu_at_window_origin(handle)
        {
            return Some(0);
        }
    ...
    fn handle_system_command(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        if is_system_command(wparam, SC_KEYMENU as usize) {
            let mut lock = self.state.borrow_mut();
            ...
            if is_alt_space_system_menu(wparam, lparam)
                && self.show_system_menu_at_window_origin(handle)
            {
                return Some(0);
            }
...
fn is_system_command(wparam: WPARAM, command: usize) -> bool {
    (wparam.0 & 0xfff0) == command
}

fn is_alt_space_system_menu(wparam: WPARAM, lparam: LPARAM) -> bool {
    is_system_command(wparam, SC_KEYMENU as usize) && lparam.loword() == VK_SPACE.0 as u16
}

fn is_alt_space_syskeydown(wparam: WPARAM, modifiers: Modifiers) -> bool {
    wparam.0 == VK_SPACE.0 as usize && modifiers.alt && !modifiers.control
}
...
mod tests {
    ...
    #[test]
    fn system_command_masks_low_nibble() {
        assert!(is_system_command(
            WPARAM((SC_KEYMENU | 0x2) as usize),
            SC_KEYMENU as usize
        ));
    }

    #[test]
    fn alt_space_detection_requires_space() {
        assert!(is_alt_space_system_menu(
            WPARAM(SC_KEYMENU as usize),
            LPARAM(VK_SPACE.0 as isize),
        ));
        assert!(!is_alt_space_system_menu(
            WPARAM(SC_KEYMENU as usize),
            LPARAM('F' as isize),
    ...
    #[test]
    fn alt_space_syskeydown_requires_alt_modifier_and_space_vk() {
        assert!(is_alt_space_syskeydown(
