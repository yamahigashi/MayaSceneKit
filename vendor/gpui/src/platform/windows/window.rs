...
impl WindowsWindowInner {
    ...
    pub(crate) fn show_system_menu_at_window_origin(&self, handle: HWND) -> bool {
        let mut window_rect = RECT::default();
        unsafe {
            if GetWindowRect(handle, &mut window_rect).is_err() {
                log::error!("GetWindowRect failed while opening the system menu");
                return false;
            }
        }

        self.show_system_menu_at_screen_point(POINT {
            x: window_rect.left,
            y: window_rect.top,
        })
    }

    pub(crate) fn show_system_menu_at_screen_point(&self, position: POINT) -> bool {
        let menu = unsafe { GetSystemMenu(self.hwnd, false) };
        if menu.0.is_null() {
            log::error!("GetSystemMenu returned null");
            return false;
        }

        unsafe {
            SetForegroundWindow(self.hwnd).ok().log_err();
...
impl PlatformWindow for WindowsWindow {
    ...
    fn show_window_menu(&self, position: Point<Pixels>) {
        ...
        let mut position = POINT {
            x: (position.x.0 * scale_factor).round() as i32,
            y: (position.y.0 * scale_factor).round() as i32,
        };

        unsafe { ClientToScreen(self.0.hwnd, &mut position).ok().log_err() };
        self.0.show_system_menu_at_screen_point(position);
    }
