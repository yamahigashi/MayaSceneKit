...
impl PopupMenu {
    ...
    pub fn replace_menu_items(&mut self, items: Vec<PopupMenuItem>) {
        self.menu_items = items;
        self.selected_index = None;
        self.scrollable = self.menu_items.len() > 20;
    }
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_mouse_down_out(cx.listener(|this, ev: &MouseDownEvent, window, cx| {
                ...
                if let Some(active_submenu) = this.active_submenu() {
                    if active_submenu.read(cx).bounds.contains(&ev.position) {
                        return;
                    }
                    this.selected_index = None;
                }
