...
impl InputState {
    ...
    ///
    /// Read-only inputs keep focus, selection, scrolling, and copy-related actions,
    /// but reject text mutations initiated by user actions.
    pub fn read_only(mut self, read_only: bool) -> Self {
        self.read_only = read_only;
        self
    }

    /// Update the read-only state of the input field.
    ...
    #[inline]
    pub(super) fn blocks_text_mutation(&self) -> bool {
...
mod tests {
    ...
    use gpui::{AppContext as _, ClipboardItem, Empty, Entity, TestAppContext};

    use super::{Backspace, InputState, Paste, SelectAll};

    fn test_input_state(
        cx: &mut TestAppContext,
        build: impl FnOnce(&mut gpui::Window, &mut gpui::Context<InputState>) -> InputState,
    ) -> (Entity<InputState>, &mut gpui::VisualTestContext) {
        let slot = Rc::new(RefCell::new(None));
        let slot_for_build = slot.clone();
        let (_, visual_cx) = cx.add_window_view(|window, cx| {
            let state = cx.new(|cx| build(window, cx));
            slot_for_build.replace(Some(state));
            Empty
        });
        (
            slot.borrow()
                .clone()
                .expect("input state should be created"),
            visual_cx,
        )
    }

    #[gpui::test]
    fn read_only_input_blocks_user_mutations(cx: &mut TestAppContext) {
        let (state, visual_cx) = test_input_state(cx, |window, cx| {
            InputState::new(window, cx)
                .multi_line(true)
                .default_value("hello")
                .read_only(true)
        });
        visual_cx.write_to_clipboard(ClipboardItem::new_string(" world".to_string()));

        state.update_in(visual_cx, |state, window, cx| {
            state.select_all(&SelectAll, window, cx);
            state.backspace(&Backspace, window, cx);
            assert_eq!(state.value().to_string(), "hello");
    ...
    #[gpui::test]
    fn set_value_updates_read_only_input(cx: &mut TestAppContext) {
        let (state, visual_cx) = test_input_state(cx, |window, cx| {
            InputState::new(window, cx)
                .multi_line(true)
        ...
        state.update_in(visual_cx, |state, window, cx| {
            ...
            assert_eq!(state.value().to_string(), "after");
